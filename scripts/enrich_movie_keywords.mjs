...
const THEMATIC_KEYWORD_PATTERNS = [
  ...
  // Killer, Assassin, Hitman, Gun-Fu, Explosions & High-Octane Action
  {
    ...
    tags: ['киллер', 'наемный убийца', 'убийца', 'убийства', 'перестрелки', 'взрывы', 'боевик', 'экшен', 'кровавый экшен', 'месть', 'джон уик', 'леон', 'драйв', 'стрельба']
  },
  ...
  {
    regex: /(^|[^а-яa-z0-9])(детектив|расследован|улики|маньяк|полицейск|следовател|шериф)/iu,
    tags: ['детектив', 'расследование', 'маньяк', 'полиция', 'тайны', 'триллер', 'мрачный']
  },
  {
...
const DIRECTOR_KEYWORDS = {
  ...
  'Квентин Тарантино': ['тарантино', 'квентин тарантино', 'культовые диалоги', 'черный юмор', 'нелинейный сюжет', 'самурайский экшен', 'катана', 'перестрелки', 'киллер', 'боевик', 'экшен'],
...
function determineIsBW(m) {
  const knownBWTitles = [
    ...
    'Летят журавли', 'Девчата', 'В бой идут одни «старики»', 'Собачье сердце', 'Шерлок младший'
  ];
...
const enrichedMovies = movies.map((m) => {
  ...
  if ([333, 28, 138, 74, 286, 127, 257, 296, 297, 182, 106, 331, 9, 319, 45, 332, 348, 6, 57, 103, 50, 824].includes(m.id)) {
    keywordSet.add('киллер');
    keywordSet.add('наемный убийца');
    keywordSet.add('убийца');
    keywordSet.add('убийства');
    keywordSet.add('перестрелки');
    keywordSet.add('взрывы');
    keywordSet.add('боевик');
    keywordSet.add('экшен');
    keywordSet.add('кровавый экшен');
    keywordSet.add('месть');
    keywordSet.add('джон уик');
    keywordSet.add('леон');
    keywordSet.add('драйв');
    keywordSet.add('стрельба');
  }

  // Explicit additions for Samurai, Katana, Kurosawa
  if ([33, 165, 128, 112, 138].includes(m.id)) {
