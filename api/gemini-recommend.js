...
export function retrieveTop50Candidates(prompt = '', userTasteVector = null, catalog = movies) {
  ...
  // 1. HARD FORMAT & ORIGIN CONSTRAINTS
  let candidatePool = [...catalog];

  const COUNTRY_ADJECTIVES_MAP = [
    { adj: 'француз', country: 'Франция' },
    { adj: 'японск', country: 'Япония' },
    { adj: 'советск', country: 'СССР' },
    { adj: 'ссср', country: 'СССР' },
    { adj: 'итальянск', country: 'Италия' },
    { adj: 'британск', country: 'Великобритания' },
    { adj: 'английск', country: 'Великобритания' },
    { adj: 'немецк', country: 'Германия' },
    { adj: 'испанск', country: 'Испания' },
    { adj: 'корейск', country: 'Корея' },
    { adj: 'скандинавск', country: 'Швеция' }
  ];

  for (const { adj, country } of COUNTRY_ADJECTIVES_MAP) {
    if (normPrompt.includes(adj)) {
      const filtered = candidatePool.filter((m) =>
        (m.country || '').includes(country) ||
        (country === 'СССР' && (m.era || '').includes('советск'))
      );
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const systemPrompt = `Ты — MatchWatch AI Cinema Genie, ведущий мировой кинокритик и кино-сомелье с безупречным кинематографическим вкусом.

ТВОЯ ЗАДАЧА:
Зритель обратился к тебе с запросом: «${cleanPrompt}».
Из предложенного списка кандидатов нашей базы выбери до ${targetCount} НАИБОЛЕЕ ТОЧНО СООТВЕТСТВУЮЩИХ запросу фильмов/произведений, выстрой их в идеальном порядке (от сильнейших рекомендаций к интересным авторским находкам) и напиши к каждому фильму сочную 1–2 предложения персональную рецензию ("reason").

УНИВЕРСАЛЬНЫЕ ПРАВИЛА ОТБОРА (КРИТИЧЕСКИ ВАЖНО):
1. АБСОЛЮТНОЕ СООТВЕТСТВИЕ ФОРМАТУ И ЖАНРУ:
   - Если в запросе зрителя прямо или косвенно задан формат (мультфильм / анимация, игровое кино, аниме, сериал, ч/б, документальное и т.д.), выбирай ИСКЛЮЧИТЕЛЬНО картины, строго соответствующие этому формату!
   - Категорически запрещено предлагать произведения несовместимого формата (например, рекомендовать игровое кино, когда запрошена анимация, или цветной фильм, когда запрошено чёрно-белое кино).
2. СМЫСЛОВОЙ И СЮЖЕТНЫЙ ФОКУС:
   - Все ключевые требования зрителя (персонажи, профессия героя, место действия, сюжетные тропы, животные, космос, наёмники, расследования, магия, ограбления и т.д.) должны являться центральными элементами сюжета выбранных картин.
3. ТОНАЛЬНОСТЬ, ВАЙБ И ЭПОХА:
   - Точно передавай желаемую тональность (уютное/доброе, мрачное/жестокое, динамичное/адреналиновое, интеллектуальное/головоломка) и временной период (детство/ностальгия, классика, современные новинки).
4. ВЫБОР ТОЛЬКО ИЗ СПИСКА КАНДИДАТОВ:
   - Используй исключительно переданные в списке ID и фильмы (не выдумывай посторонние ID).
5. РЕЦЕНЗИЯ ("reason"):
   - В 1–2 предложениях подчеркни, почему именно эта картина идеально отвечает на запрос зрителя, упомянув её ключевую изюминку, атмосферу или сюжетный стержень.
...
ФОРМАТ ВЫВОДА (СТРОГО JSON):
{
  "recommendations": [
    {
      "id": 123,
      "reason": "1-2 ярких предложения кинокритика о том, почему этот фильм идеально подходит под запрос."
    }
  ],
  "aiSummary": "Ёмкое резюме подборки под настроение зрителя..."
}`;
