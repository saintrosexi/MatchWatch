...
async function main() {
  ...
  // Remove bad movies
  const badTitles = ["Можешь рассчитывать на меня", "Хакеры", "Пещера колдунов", "Запомни это"];
  movies = movies.filter(m => !badTitles.includes(m.titleRu));
