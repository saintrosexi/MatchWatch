...
-- та же: тариф меняют по результатам, а не выкладкой. Стартовые значения
-- миграция кладёт сама (в конце файла), иначе «три комнаты бесплатно»
-- существовало бы только в тексте витрины.
--
-- Премиум при этом никому не выдаётся молча: на время первой волны его
-- может активировать любой, но за ноль рублей и НАЖАТИЕМ. Выданный
-- молча премиум не сообщает ничего — человек его не заметил.
--
...
as $$
  select
    /*
     * По умолчанию — НЕ выдавать. Премиум берётся нажатием (на время
     * первой волны за ноль рублей), а не появляется сам. Прежний
     * умолчательный `true` означал «премиум у всех, пока не настроим»,
     * то есть ровно ту модель, от которой мы отказались.
     */
    coalesce((public.premium_settings() ->> 'grantAllUsers')::boolean, false)
    or exists (
...
/*
 * Значения тарифа на старте первой волны.
 *
 * Кладём строкой, а не оставляем на «настроим потом»: без неё лимит
 * не действует вовсе, и «три комнаты бесплатно» существовало бы только
 * в тексте витрины. `do nothing` — чтобы миграция не затирала значения,
 * которые к тому моменту уже подкрутили руками.
 */
insert into public.app_config (key, value)
values ('premium', '{"grantAllUsers": false, "freeLimits": {"roomsPerMonth": 3}}'::jsonb)
